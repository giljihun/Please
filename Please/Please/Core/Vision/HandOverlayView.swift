...
/// #13 문턱값 조정이 끝나면 제거하거나 개발자 설정 뒤로 숨긴다
final class HandOverlayView: UIView {
    ...
    private func drawPenTip(pose: HandPose, in context: CGContext) {
        ...
        text.draw(at: CGPoint(x: point.x + radius + 4, y: point.y - 8), withAttributes: attributes)
    }

    // MARK: - 좌표 변환

    /// 제스처 드로잉과 같은 변환기를 쓴다 — 링과 실제 선이 어긋나면 진단이 무의미해진다
    private func screenPoint(_ normalized: CGPoint) -> CGPoint? {
        VisionCoordinateMapper(imageSize: imageSize, viewSize: bounds.size)
